...
face3DPos ::
    (PointND a, Buildable simplex Vec3, Sub simplex ~ S1) =>
...
extractAllSimplexFaces sP sigma = map toSimplexFace fsAll
  where
    ...
    toSimplexFace fp@(f, x) =
        let
            nd = outterND fp
         in
            ActiveUnit{activeUnit = Face3D f, assocP = x, assocND = nd}
    outterND ((na, nb, nc), x) =
        let
            nd = normalize (sP !. nb &- sP !. na) &^ (sP !. nc &- sP !. na)
         in
            if (sP !. na &- sP !. x) &. nd > 0 then neg nd else nd
