...
instance PointND Vec3 where
    ...
    isInBox box (Vec3 x y z) =
        let
         ...
         in
            between (xMin3D box) (xMax3D box) x
  ...
  where
    ...
    findThird =
        let
         ...
         in
            findMinimunButZero' dist justHull
    ...
    isHull x
        ...
        | otherwise =
            let
             ...
             in
                case calcPlane sP face of
                    Just plane
                        | (L.null . pointsOnB1) pp -> True
                        | (L.null . pointsOnB2) pp -> True
                        | otherwise -> False
                      where
                        pp = pointSetPartition (whichSideOfPlane plane) sP (cleanList x)
                    Nothing -> False
        ...
        getIn2D plane =
            let
             ...
             in
                ...
                fst $ getFaceDistCenter pA pB pC
     in
        getIn2D <$> calcPlane sp face
