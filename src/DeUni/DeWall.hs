...
runHull3D box sP ps =
    let
        initi = initState sP
     in
        runState (mbc ps (S.empty :: SetActiveSubUnits S1 Vec3) box []) initi

runDelaunay3D :: Box Vec3 -> SetPoint Vec3 -> [PointPointer] -> (SetSimplex3D, StateVarsMBC S2 Vec3)
runDelaunay3D box sP ps =
    let
        initi = initState sP
     in
        runState (mbc ps (S.empty :: SetActiveSubUnits S2 Vec3) box []) initi

runDelaunay2D :: Box Vec2 -> SetPoint Vec2 -> [PointPointer] -> (SetSimplex2D, StateVarsMBC S2 Vec2)
runDelaunay2D box sP ps =
    let
        initi = initState sP
     in
        runState (mbc ps (S.empty :: SetActiveSubUnits S2 Vec2) box []) initi

runDelaunay :: (Buildable S2 a) => Box a -> SetPoint a -> [PointPointer] -> (IntMap (S2 a), StateVarsMBC S2 a)
runDelaunay box sP ps =
    let
        initi = initState sP
     in
        runState (mbc ps S.empty box []) initi
...
mbc p afl box subbox = do
    ...
    units <-
        if S.null afl -- same as null aflAlpha && null aflBox1 && null aflBox2
