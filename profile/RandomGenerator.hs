...
instance GenRandom Point2D where
    ...
    calcBoxSize avgVolume (xRatio, yRatio) =
        let
            refRatio = xRatio
            k1 = yRatio / refRatio
            a = (avgVolume / k1) ** (1 / 2)
         in
            Box2D{xMax2D = a, xMin2D = 0, yMax2D = a * k1, yMin2D = 0}
...
instance GenRandom Point3D where
    ...
    calcBoxSize avgVolume (xRatio, yRatio, zRatio) =
        let
         ...
         in
            Box3D{xMax3D = a, xMin3D = 0, yMax3D = a * k1, yMin3D = 0, zMax3D = a * k2, zMin3D = 0}
...
genFullRandomGrainDistribution gen targetNGrains avgVolume dist ratio =
    let
     ...
     in
        replicateM targetNGrains getPoint >>= return . (DistributedPoints box) . fromList
